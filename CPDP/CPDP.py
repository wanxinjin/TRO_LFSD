...
class COCSys:
    ...
    def cocSolver(self, ini_state, horizon, auxvar_value=1, interplation_level=1, print_level=0, warmup_start=True):
        ...
        # Solve the NLP
        if warmup_start is True and self.prev_coc_sol is not None:
            w0 = self.prev_coc_sol
        sol = solver(x0=w0, lbx=lbw, ubx=ubw, lbg=lbg, ubg=ubg)
        w_opt = sol['x'].full().flatten()
        self.prev_coc_sol = w_opt
        ...
        opt_sol = self.interpolation(time_grid,
                                     numpy.concatenate((state_grid_opt, control_grid_opt, costate_grid_opt), axis=1),
                                     interplation_level)
    ...
    def raccatiODE(self):
        ...
        P_dot = -(Q + casadi.mtimes(A.T, self.P) + casadi.mtimes(self.P, A) - casadi.mtimes(casadi.mtimes(self.P, R),
                                                                                            self.P))
        W_dot = casadi.mtimes(casadi.mtimes(self.P, R), self.W) - casadi.mtimes(A.T, self.W) - casadi.mtimes(self.P,
                                                                                                             r) - q

        self.raccati_fn = Function('Pdot', [self.state, self.control, self.costate, self.auxvar, self.P, self.W],
                                   [P_dot, W_dot])
    ...
    def auxSysODE(self):
        ...
        invHuu = casadi.pinv(self.ddHuu)
        auxsys_control = -mtimes(invHuu,
                                 mtimes((self.ddHux + self.dfu.T @ self.P), self.auxsys_state) + mtimes(self.dfu.T,
                                                                                                        self.W) + self.ddHue)
        self.auxsys_controller_fn = Function('auxsys_controller',
                                             [self.state, self.control, self.costate, self.auxvar, self.P, self.W,
                                              self.auxsys_state], [auxsys_control])
        auxsys_state_dot = mtimes(self.dfx, self.auxsys_state) + mtimes(self.dfu, auxsys_control) + self.dfe
        self.auxsys_state_dot_fn = Function('auxsys_x_dot',
                                            [self.state, self.control, self.costate, self.auxvar, self.P, self.W,
                                             self.auxsys_state], [auxsys_state_dot])
    ...
    def auxSysSolver(self, time_grid, opt_sol, auxvar_value=1):
        ...
        P = numpy.reshape(vec_PW[0:self.n_state * self.n_state], (self.n_state, self.n_state))
        W = numpy.reshape(vec_PW[self.n_state * self.n_state:], (self.n_state, -1))
        auxsys_control = self.auxsys_controller_fn(x, u, lam, auxvar_value, P, W,
                                                   vec_auxsys_state[0, :].reshape((self.n_state, self.n_auxvar)))
        ...
        for k in range(0, self.n_grid):
            ...
            if len(intSol.y) > 1:
                ...
                vec_auxsys_state[k + 1, :] = vec_auxsys_state[k, :]
            ...
            W = numpy.reshape(vec_PW[self.n_state * self.n_state:], (self.n_state, -1))
            auxsys_control = self.auxsys_controller_fn(x, u, lam, auxvar_value, P, W,
                                                       vec_auxsys_state[k + 1, :].reshape(
                                                           (self.n_state, self.n_auxvar)))
...
class COCSys_TimeVarying:
    ...
    def setPathCost(self, path_cost):
        ...
        self.path_cost = path_cost
        self.path_cost_fn = casadi.Function('path_cost', [self.time, self.state, self.control, self.auxvar],
                                            [self.path_cost])
    ...
    def diffPMP(self):
        ...
        # first order differential of path Hamiltonian
        self.dHx = jacobian(self.path_Hamil, self.state).T
        self.dHx_fn = casadi.Function('dHx', [self.time, self.state, self.control, self.costate, self.auxvar],
                                      [self.dHx])
        self.dHu = jacobian(self.path_Hamil, self.control).T
        self.dHu_fn = casadi.Function('dHu', [self.time, self.state, self.control, self.costate, self.auxvar],
                                      [self.dHu])
        # second order differential of path Hamiltonian
        self.ddHxx = jacobian(self.dHx, self.state)
        self.ddHxx_fn = casadi.Function('ddHxx', [self.time, self.state, self.control, self.costate, self.auxvar],
                                        [self.ddHxx])
        self.ddHxu = jacobian(self.dHx, self.control)
        self.ddHxu_fn = casadi.Function('ddHxu', [self.time, self.state, self.control, self.costate, self.auxvar],
                                        [self.ddHxu])
        self.ddHxe = jacobian(self.dHx, self.auxvar)
        self.ddHxe_fn = casadi.Function('ddHxe', [self.time, self.state, self.control, self.costate, self.auxvar],
                                        [self.ddHxe])

        self.ddHux = jacobian(self.dHu, self.state)
        self.ddHux_fn = casadi.Function('ddHux', [self.time, self.state, self.control, self.costate, self.auxvar],
                                        [self.ddHux])
        self.ddHuu = jacobian(self.dHu, self.control)
        self.ddHuu_fn = casadi.Function('ddHuu', [self.time, self.state, self.control, self.costate, self.auxvar],
                                        [self.ddHuu])
        self.ddHue = jacobian(self.dHu, self.auxvar)
        self.ddHue_fn = casadi.Function('ddHue', [self.time, self.state, self.control, self.costate, self.auxvar],
                                        [self.ddHue])
    ...
    def raccatiODE(self):
        ...
        self.raccati_fn = Function('Pdot',
                                   [self.time, self.state, self.control, self.costate, self.auxvar, self.P, self.W],
                                   [P_dot, W_dot])
    ...
    def auxSysODE(self):
        ...
        auxsys_control = -mtimes(invHuu,
                                 mtimes((self.ddHux + self.dfu.T @ self.P), self.auxsys_state) + mtimes(self.dfu.T,
                                                                                                        self.W) + self.ddHue)
        self.auxsys_controller_fn = Function('auxsys_controller',
                                             [self.time, self.state, self.control, self.costate, self.auxvar, self.P,
                                              self.W, self.auxsys_state], [auxsys_control])
        auxsys_state_dot = mtimes(self.dfx, self.auxsys_state) + mtimes(self.dfu, auxsys_control) + self.dfe
        self.auxsys_state_dot_fn = Function('auxsys_x_dot',
                                            [self.time, self.state, self.control, self.costate, self.auxvar, self.P,
                                             self.W, self.auxsys_state], [auxsys_state_dot])
    ...
    def auxSysSolver(self, time_grid, opt_sol, auxvar_value=1):
        ...
        P = numpy.reshape(vec_PW[0:self.n_state * self.n_state], (self.n_state, self.n_state))
        W = numpy.reshape(vec_PW[self.n_state * self.n_state:], (self.n_state, -1))
        auxsys_control = self.auxsys_controller_fn(0, x, u, lam, auxvar_value, P, W,
                                                   vec_auxsys_state[0, :].reshape((self.n_state, self.n_auxvar)))
        vec_auxsys_control[0, :] = auxsys_control.full().flatten()
        ...
        for k in range(0, self.n_grid):
            ...
            P = numpy.reshape(vec_PW[0:self.n_state * self.n_state], (self.n_state, self.n_state))
            W = numpy.reshape(vec_PW[self.n_state * self.n_state:], (self.n_state, -1))
            auxsys_control = self.auxsys_controller_fn(time_grid[k + 1], x, u, lam, auxvar_value, P, W,
                                                       vec_auxsys_state[k + 1, :].reshape(
                                                           (self.n_state, self.n_auxvar)))
            vec_auxsys_control[k + 1, :] = auxsys_control.full().flatten()
