...
for k in range(5,max_trial):
    ...
    for j in range(int(200)):
        ...
        loss_trace += [loss]
        parameter_trace += [current_parameter.tolist()]
        print('case2:', 'trial:', k, 'iter:', j, 'loss:', loss_trace[-1].tolist())
        if loss<2: optimizier.learning_rate=1e-3
    ...
    if True:
        save_data = {'loss_trace': loss_trace,
                     ...
                     'T': T}
        np.save('./results/case_2/quadrotor_trial_'+str(k)+'.npy', save_data)
