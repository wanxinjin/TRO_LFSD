...
for k in range(6,max_trial):

    optimizier = opt.Adam()
    optimizier.learning_rate = 10e-2
    loss_trace, parameter_trace = [], []
    current_parameter = np.random.uniform(1,2,oc.n_auxvar)
    ...
    for j in range(int(100)):
        ...
        print('case4:', 'trial:', k, 'iter:', j, 'loss:', loss_trace[-1].tolist(), 'loss_test:', loss_test)
        if loss<10: optimizier.learning_rate=1e-2
    ...
    if True:
        save_data = {'loss_trace': loss_trace,
                     ...
                     'T': T}
        np.save('./results/case_4/quadrotor_trial_' + str(k) + '_2.npy', save_data)
