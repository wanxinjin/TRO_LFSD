...
def getloss(time_grid, target_waypoints, opt_sol):
    loss = 0
    diff_loss = numpy.zeros(oc.n_auxvar)
    for k, t in enumerate(time_grid):
        # solve loss
...
waypoints = np.array([[-4, -6., 3],
                      ...
                      [-1.0, 1., 5],
                      [2.0, 3.0, 4.0]])
taus = taus + np.random.uniform(-0.05, 0.05, taus.size)
...
for k in range(1, max_trial):
    ...
    current_parameter = current_parameter + np.random.uniform(0, 1, oc.n_auxvar)
    ...
    for j in range(int(200)):
        ...
        # current_parameter = current_parameter - learning_rate * diff_loss
        loss_trace += [loss]
        parameter_trace += [current_parameter.tolist()]

        time_grid_test, opt_sol_test = oc.cocSolver(ini_state, T, current_parameter, warmup_start=False)
        loss_test = getloss(taus, waypoints, opt_sol)

        print('case5:', 'trial:', k, 'iter:', j, 'loss:', loss_trace[-1].tolist(), 'loss_test:', loss_test)
        if loss < 20: optimizier.learning_rate = 1e-2

    # save the true
    # Below is to obtain the final uav trajectory based on the learned objective function
    _, opt_sol = oc.cocSolver(ini_state, T, current_parameter, warmup_start=False)
    loss_test = getloss(taus, waypoints, opt_sol)
    ...
    if True:
        save_data = {'loss_trace': loss_trace,
                     ...
                     'T': T}
        np.save('./results/case_5/quadrotor_trial_' + str(k) + '_2.npy', save_data)
