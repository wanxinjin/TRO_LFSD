...
for k in range(2,max_trial):
    ...
    for j in range(int(200)):
        ...
        loss_trace += [loss]
        parameter_trace += [current_parameter.tolist()]
        print('case feature, subcase 2:', 'trial:', k, 'iter:', j, 'loss:', loss_trace[-1].tolist())
        if loss<30: optimizier.learning_rate=1e-2
        #
        # # this is to avoid non solution for the optimal control (without penalty on the control effort)
        if current_parameter[1] < 0.01: current_parameter[1] = 0.01
        if current_parameter[2] < 0.01: current_parameter[2] = 0.01
    ...
    if True:
        save_data = {'loss_trace': loss_trace,
                     ...
                     'T': T}
        np.save('./results/case_feature/subcase2_trial_'+str(k)+'.npy', save_data)
