...
class RobotArm:
    ...
    def play_animation_ex(self, l1, l2, dt, state_traj, state_traj_ref=None, save_option=0, horizon=1):
        ...
        y2 = 1 * sin(q1 + q2) + y1
        alpha = 0.5
        line_waypoint1, = ax.plot([], [], linewidth=15, marker='o', color='gray', markersize=20,
        ...
        line1, = ax.plot([], [], linewidth=15, marker='o', color='#0072BD', markersize=20, markerfacecolor='black',
                         markeredgecolor='red', markeredgewidth=5, zorder=10000, alpha=1)
...
class Quadrotor:
    ...
    def play_animation(self, wing_len, state_traj, state_traj_ref=None, dt=0.1, save_option=0, title='UAV Maneuvering',
        ...
        ax.scatter(waypoints[:, 0], waypoints[:, 1], waypoints[:, 2], s=56, color='red', alpha=0, marker='^')
        ...
        if waypoints is not None:
            ax.scatter(waypoints[:, 0], waypoints[:, 1], waypoints[:, 2], s=80, color='red', alpha=1,
                       marker='^')
        ...
        # animation
        line_traj, = ax.plot(position[:1, 0], position[:1, 1], position[:1, 2], zorder=0.1, )
        ...
        #
        # bar2 = ax.bar3d([-1], [-4], [1], dx=[0.5], dy=[0.5], dz=[3.5], color='#D95319',alpha=1, zorder = 1)
        # bar22 = ax.bar3d([-1], [-4], [4], dx=[0.5], dy=[-3.5], dz=[0.5], color='#D95319',alpha=1,zorder = 1)
        # bar23 = ax.bar3d([-1], [-7.5], [4], dx=[0.5], dy=[0.5], dz=[-3], color='#D95319',alpha=1,zorder = 1)
        # bar24 = ax.bar3d([-1], [-4], [1], dx=[0.5], dy=[-3.5], dz=[0.5], color='#D95319',alpha=1,zorder = 1)
        # bar1 = ax.bar3d([0.5], [0], [3.5], dx=[0.5], dy=[0.5], dz=[3], color='#D95319',alpha=1,)
        # bar11 = ax.bar3d([0.5], [0], [3.5], dx=[0.5], dy=[4.5], dz=[0.5], color='#D95319',alpha=1,zorder = 1)
        # bar12 = ax.bar3d([0.5], [0], [6.5], dx=[0.5], dy=[4.5], dz=[0.5], color='#D95319',alpha=1,zorder = 1)
        # bar13 = ax.bar3d([0.5], [4], [7.0], dx=[0.5], dy=[0.5], dz=[-3.5], color='#D95319',alpha=1,zorder = 1)
    ...
    def play_animation2(self, wing_len, state_traj, state_traj_ref=None, dt=0.1, save_option=0, title='UAV Maneuvering',
                        horizon=1, waypoints=None):
        ...
        ax.set_ylim(-9, 9)
        ax.set_xlim(-9, 9)
        ax.set_title('New goal', pad=0, fontsize=30, fontweight='bold')
        ax.view_init(elev=27, azim=-64)
        ...
        # ax.view_init(elev=88, azim=-90)
        # ax.set_title('Top view', fontsize=40, pad=-25)
        # ax.set_position([-0.17, -0.05, 1.30, 1.0])
        # #
        # fig = plt.figure(figsize=(7,6))
        # ax = fig.add_subplot(1, 1, 1, projection='3d', )
        # ax.set_xlabel('X', fontsize=40, labelpad=10)
        ...
        # ax.tick_params(labelbottom=False, labelright=False, labelleft=False)
        # ax.view_init(elev=0, azim=-90)
        # ax.set_title('Front view', fontsize=40, pad=-100)
        # ax.set_position([-0.17, -0.20, 1.30, 1.3])


        if waypoints is not None:
            ax.scatter(waypoints[:, 0], waypoints[:, 1], waypoints[:, 2], s=80, color='tab:green', alpha=1, zorder=1000,
                       marker='^')
        # data
        position = self.get_quadrotor_position(wing_len, state_traj)
        sim_horizon = np.size(position, 0)
        ...
        ax.scatter(position[-1, 0], position[-1, 1], position[-1, 2], s=160, color='green', alpha=1, zorder=1000,
                   marker='*')



        # animation
        line_traj, = ax.plot(position[:1, 0], position[:1, 1], position[:1, 2], zorder=0.1, lw=2)
        c_x, c_y, c_z = position[0, 0:3]
        r1_x, r1_y, r1_z = position[0, 3:6]
        r2_x, r2_y, r2_z = position[0, 6:9]
        r3_x, r3_y, r3_z = position[0, 9:12]
        ...
        gate1_back, = ax.plot([-1, -1], [-3, -3], [1.5, 4], lw=7, color='#D95319', zorder=-100)
        gate1_up, = ax.plot([-1, -1], [-3, -6.5], [4, 4], lw=7, color='#D95319', )
        gate1_front, = ax.plot([-1, -1], [-6.5, -6.5], [1.5, 4], lw=7, color='#D95319', zorder=100)
        gate1_down, = ax.plot([-1, -1], [-3, -6.5], [1.5, 1.5], lw=7, color='#D95319', zorder=-100)

        gate2_back, = ax.plot([0.5, 0.5], [4, 4], [4., 6.5], lw=7, color='#D95319', zorder=-100)
        gate2_up, = ax.plot([0.5, 0.5], [0, 4], [6.5, 6.5], lw=7, color='#D95319', zorder=100)
        gate2_front, = ax.plot([0.5, 0.5], [0, 0], [4., 6.5], lw=7, color='#D95319', zorder=100)
        gate2_down, = ax.plot([0.5, 0.5], [0, 4], [4., 4.], lw=7, color='#D95319', zorder=-100)
        ...
        def update_traj(num):
            ...
            # plot gate
            # gate1_up.set_data(np.array([[-1, -1], [-4, -7.5]]))
            # gate1_up.set_3d_properties(np.array([5.5, 5.5]))
            # gate1_back.set_data(np.array([[-1, -1],[-4, -4]]))
            # gate1_back.set_3d_properties(np.array([1, 5.5]))
            # gate1_front.set_data(np.array([[-1, -1],[-7.5, -7.5]]))
            # gate1_front.set_3d_properties(np.array([1, 5.5]))
            # gate1_down.set_data(np.array([[-1, -1],[-4, -7.5]]))
            # gate1_down.set_3d_properties(np.array([1, 1]))

            return line_traj, line_arm1, line_arm2, line_arm3, line_arm4, \
                   gate1_back, gate1_up, gate1_front, gate1_down, \
                   gate2_back, gate2_up, gate2_front, gate2_down
        ...
        if save_option != 0:
            Writer = animation.writers['ffmpeg']
            writer = Writer(fps=10, metadata=dict(artist='Me'), bitrate=-1)
            ani.save('video.mp4', writer=writer, dpi=300)
            print('save_success')
